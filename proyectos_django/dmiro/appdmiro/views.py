...
def fusionLines(request,agencia,anio):
    ...
    for asesor in queryset_asesores:
        ...
        if query:
            dataset = {}
            dataset['seriesname'] = "ID: "+str(asesor.id) + ", "+"NAME: " + asesor.ase_nombres +" "+asesor.ase_apellidos           
            dataset['data'] = []
...
def fusionBarras(request):
    ...
    datasource['chart']={"caption": "Monthly revenue for last year",
                ...
                "subcaptionFontBold": "0",
                "subcaptionFontSize": "14"}
    for valores in queryset:
        data = {}
        data['label'] = valores.nombre
        data['value'] = valores.coordenadax
